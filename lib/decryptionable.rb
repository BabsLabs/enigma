module Decryptionable
  ...
  def decryption_helper(cyphertext, key, date)
    ...
    downcase_split_message.each do |letter|
      ...
      if alphabet_array.include?(letter)
        ...
        new_letter = alphabet_array.rotate((shift_array[0] * -1) + alphabet_array.index(letter))
    ...
    # assign the keys and values to the decryption_hash
